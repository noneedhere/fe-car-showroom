...
const DeleteSale = ({ selectedSale }: { selectedSale: Sale }) => {
    const [isShow, setIsShow] = useState<boolean>(false)
    const [sale, setSale] = useState<Sale>({ ...selectedSale })
    ...
    const handleSubmit = async (e: FormEvent) => {
        e.preventDefault()
        try {
            const url = `/sale/${selectedSale.id_sale}`
        ...
        <div>
            ...
            <Modal isShow={isShow} onClose={(state) => setIsShow(state)}>
                <form onSubmit={handleSubmit}>
                    ...
                    <div className="sticky top-0 bg-white px-5 pt-5 pb-3 shadow">
                        <div className="w-full flex items-center">
                            <div className="flex flex-col">
                                <strong className="font-bold text-2xl text-black">Delete Sale</strong>
                                <small className="text-slate-400 text-sm">
                                    Sales data that has been recorded cannot be recovered after deletion.
                                </small>
                            </div>
                    ...
                    {/* modal body */}
                    <div className="p-5">
                        Are you sure you want to delete the sale transaction made by <b>{sale.buyerName}</b>?
                    </div>
                    {/* end modal body */}
