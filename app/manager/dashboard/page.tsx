...
const HomePage = () => {
    ...
    useEffect(() => {
        ...
        const fetchDashboardData = async () => {
            try {
                setIsLoading(true);
                const response = await fetch(`${BASE_API_URL}/report/dashboard`);
    ...
    const statCards = [
        {
            label: "Total Cars",
            value: isLoading ? "..." : String(dashboardData?.allCars ?? 0),
            icon: (
        ...
        {
            label: "Total Users",
            value: isLoading ? "..." : String(dashboardData?.allUser ?? 0),
            icon: (
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6">
        ...
        {
            label: "Total Sales",
            value: isLoading ? "..." : String(dashboardData?.totalSales ?? 0),
            icon: (
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6">
    ...
    const quickActions = [
        {
            label: "Manage Cars",
            description: "Add, edit, or remove car inventory",
            path: "/manager/car",
            color: "bg-sky-50 text-sky-600",
            icon: (
        ...
        <div className="space-y-6">
            ...
            <div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                    {quickActions.map((action, index) => (
                        <Link key={index} href={action.path} className="quick-action">
                            ...
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-4 h-4 text-text-muted ml-auto">
            ...
            <div className="admin-card p-6">
                <div className="flex items-start gap-3">
                    ...
                    <div>
                        <h4 className="text-sm font-semibold text-text-primary">Getting Started</h4>
                        <p className="text-sm text-text-secondary mt-1">
                            Use the sidebar navigation to manage your car showroom inventory, users, and sales records.
                            Dashboard statistics will populate automatically as you add data.
                        </p>
                    </div>
                </div>
            </div>
