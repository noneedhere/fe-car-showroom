...
const DeleteUser = ({ selectedUser }: { selectedUser: User }) => {
    ...
    const handleSubmit = async (e: FormEvent) => {
        try {
            e.preventDefault()
            const url = `/user/${selectedUser.id_user}`
            const { data } = await drop(url, TOKEN)
            if (data?.status) {
                setIsShow(false)
                toast(data?.message, { containerId: `toastMenu`, type: `success`, autoClose: 3000 })
                setTimeout(() => router.refresh(), 1000)
            } else {
                toast(data?.message, { containerId: `toastMenu`, type: `warning`, autoClose: 5000 })
            }
        } catch (error) {
            console.log(error);
            toast(`Something Wrong`, { containerId: `toastMenu`, type: `error`, autoClose: 5000 })
        }
    }
    return (
        <div>
            <ToastContainer containerId={`toastMenu`} autoClose={5000} />
