...
const EditUser = ({ selectedUser }: { selectedUser: User }) => {
    ...
    const handleSubmit = async (e: FormEvent) => {
        try {
            ...
            if (data?.status) {
                setIsShow(false)
                toast(data?.message, { containerId: `toastUser`, type: `success`, autoClose: 3000 })
                setTimeout(() => router.refresh(), 1000)
            } else {
                toast(data?.message, { containerId: `toastUser`, type: `warning`, autoClose: 5000 })
            }
        } catch (error) {
            console.log(error);
            toast(`Something Wrong`, { containerId: `toastUser`, type: `error`, autoClose: 5000 })
        }
    }

    return (
        <div>
            <ToastContainer containerId={`toastUser`} autoClose={5000} />
